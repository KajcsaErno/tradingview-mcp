...
REM Check MSIX / Windows Store install via AppX package query
REM (cmd's `dir` can't enumerate %PROGRAMFILES%\WindowsApps without admin,
REM  but PowerShell's Get-AppxPackage reads it through the package manager API)
if "%TV_EXE%"=="" (
    for /f "usebackq delims=" %%i in (`powershell -NoProfile -Command "$p = Get-AppxPackage TradingView.Desktop -ErrorAction SilentlyContinue; if ($p) { Join-Path $p.InstallLocation 'TradingView.exe' }"`) do set "TV_EXE=%%i"
)
if "%TV_EXE%"=="" (
