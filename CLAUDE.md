...
- **Sizing & config:** `calcPositionSize` (pure calc), `setLeverage`, `setMarginType`, `setPositionMode`. `calcPositionSize` takes either an explicit `stop` **or** an **ATR-derived stop**: pass `atrMult` + `side` (+ `symbol`/`interval`) and it pulls ATR(14) via `getTechnicals` and sets the stop to `entry ∓ atrMult·ATR` (below for long, above for short), echoing `atrStop:{atr,atrMult,interval}` in the result.
