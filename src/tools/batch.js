...
export function registerBatchTools(server) {
    registerTool(server, 'batch_run', 'Run an action across multiple symbols and/or timeframes', {
    ...
    }, ({symbols, timeframes, action, delay_ms, ohlcv_count}) => core.batchRun({symbols, timeframes, action, delay_ms, ohlcv_count}));
}
