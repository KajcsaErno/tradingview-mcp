...
export function registerAlertTools(server) {
  ...
  server.tool('alert_activate', 'Reactivate an inactive (already-fired or stopped) alert by ID. Use alert_list to find the alert_id.', {
    alert_id: z.coerce.number().describe('Numeric alert_id from alert_list output'),
  }, async ({ alert_id }) => {
    try { return jsonResult(await core.activate({ alert_id })); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });
}
