...
export function registerIndicatorTools(server) {
    registerTool(server, 'indicator_set_inputs', 'Change indicator/study input values (e.g., length, source, period)', {
    ...
    }, ({entity_id, inputs}) => core.setInputs({entity_id, inputs}));

    registerTool(server, 'indicator_toggle_visibility', 'Show or hide an indicator/study on the chart', {
    entity_id: z.string().describe('Entity ID of the study (from chart_get_state)'),
    visible: z.coerce.boolean().describe('true to show, false to hide'),
    }, ({entity_id, visible}) => core.toggleVisibility({entity_id, visible}));
}
