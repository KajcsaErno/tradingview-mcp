...
`src/core/binance.js` is an independent module that talks to the Binance REST API (signed HMAC-SHA256). It does **not** use CDP, `evaluate()`, `safeString()`, or `KNOWN_PATHS`. It is fully wired through all three layers (40 tools / 42 CLI subcommands):
...
**DI shape differs from the CDP modules:** instead of `{ evaluate, waitForChartReady }`, Binance functions take `_deps = { fetch, now, keys, sleep }`. Tests in `tests/binance.test.js` inject these (a mock `fetch`, fixed `now`, fake `keys`, and a no-op `sleep`) to assert on the exact requests built — **no network, no real keys**. Run `npm run test:binance` (90 tests).
