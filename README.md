...
| Order flow and microstructure (`watch-flow`, `footprint`, `vol-regime`, `options-surface`)                                                                  | Read aggressive buy/sell delta and depth imbalance live, per-candle footprint aggression, realized-vol regime, and options-implied IV/skew | Adds a tape/footprint read most chart tools lack  | `npm run tv -- binance watch-flow --symbol BTCUSDC -d 12`                                                                                                   |
...
| Risk math (`position-size`, `grid-plan`, `expectancy`, `losing-streak`, `simulate-equity`)                                                                  | Size positions, plan grids, and model outcome distributions                                                                                | Prevents random sizing decisions                  | `npm run tv -- binance grid-plan --symbol BTCUSDC --lower 58000 --upper 62000 --totalNotional 3000`                                                         |
